...
protocol ProductsCellHorizontalDelegate: AnyObject {
    func didTapIncreaseButton(cell: ProductsCellHorizontal)
    func didTapDecreaseButton(cell: ProductsCellHorizontal)
}
final class ProductsCellHorizontal: UICollectionViewCell {
    
    weak var delegate: ProductsCellHorizontalDelegate?
    static let identifier = "ProductsCellHorizontal"
    ...
    func configure(with name: String, price: String, attribute: String, image: String) {
        ...
        guard let image = URL(string: image) else { return }
        
        productImage.sd_setImage(with: image) { (image, error, cacheType, imageUrl) in
            if let error = error {
...
final class ProductsCell: UICollectionViewCell {
    ...
    func configure(with name: String, price: String, attribute: String, image: String, number: Int) {
        productName.text = name.truncateToFourWords()
        productPrice.text = "\(price)"
        productAttribute.text = attribute
        count = number
